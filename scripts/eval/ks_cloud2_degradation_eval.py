"""ks_cloud2 under radio degradation — the Kashmiri deployment gate for the
...
raw (L0) and diacritic-normalised (L2). Compares ks_cloud2 vs deployed Whisper-ks
(the gate ks_max passed 4/5) AND vs ks_max itself (cached hyps) when available.

Output: eval_data/ks_cloud2_degradation_hyps.jsonl + docs/ks_cloud2_degradation.json
"""
import io
import json
import sys
...
ADAPTER_DIR = ROOT / "finetune_runs_seamless" / "ks_cloud2" / "adapter"
...
OUT_JSONL   = ROOT / "eval_data" / "ks_cloud2_degradation_hyps.jsonl"
OUT_JSON    = ROOT / "docs" / "ks_cloud2_degradation.json"
...
def main():
    ...
    print(f"[model] loading ks_cloud2 adapter on {device} ...")
        ...
        for cond in CONDITIONS:
            for w in wavs:
                ...
                fh.write(json.dumps({"condition": cond, "idx": idx,
                                    ...
                                    ensure_ascii=False) + "\n")
            print(f"[ks_cloud2] {cond}: {len(ks2[cond])} done", flush=True)
    ...
    hdr = f"\n{'cond':10} {'ruler':10} {'ks_cloud2 WER':>12} {'whisper WER':>12}"
    if have_ksmax:
        hdr += f" {'ks_max WER':>11}"
    ...
    results["_summary"] = {"ks_cloud2_beats_whisper_L2_conditions": f"{ks2_wins_vs_whisper}/{len(CONDITIONS)}"}
    print(f"\n[summary] ks_cloud2 beats Whisper-ks (L2 nodiac) in "
