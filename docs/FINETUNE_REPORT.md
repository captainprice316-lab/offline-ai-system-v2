...
¶ Kashmiri: the n=100 cross-model re-run was not repeated for ks (its loader pulls the full 18 GB IndicVoices train split). 74.02% is the **training-split validation** WER at step 2400 against a 96.87% baseline (−22.85 pp) — both on the training ruler, and not comparable with any held-out figure in this report. On the held-out 372-clip IndicVoices-R split at L2 the same model scores **65.19%**, and at L0 79.29%.  
...
| Kashmiri (ks) | — (—) | 65.19 (39.36)† | **50.26 (23.34)**† | −14.93 pp | **SeamlessM4T + LoRA◊** |

† **Kashmiri is not on this table's ruler.** Every other row is FLEURS n=100; Kashmiri is absent from FLEURS, so its figures are the **372-clip IndicVoices-R test split at L2** (diacritic-normalised) — fine-tuned Whisper-ks 65.19% against the deployed `ks_cloud3` SeamlessM4T adapter 50.26%, a paired-bootstrap gap of −14.93 pp, 95% CI [−17.26, −12.62], p < 0.001. This row previously read `96.87 (—) | 74.02 (—)`, which are **training-split validation** figures (and 96.87 is a *different model*, `muneebharoon/whisper-small-ks`) sitting in a table of held-out ones. No un-fine-tuned Kashmiri baseline was ever measured on the held-out split, hence the dash. Read this row down, not across to its neighbours. Note the "Whisper Fine-Tuned" column is now the *rollback* model, not the deployed one — see ◊.
...
- **Pipeline role:** `ks` → SeamlessM4T v2 + ks_max LoRA adapter → NLLB-200 → English. Its trainable `__kas__` embedding delta is baked into the shared base model's embedding matrix once at load time (a fixed correction — nothing trains further at inference), so the adapter's remaining LoRA rank-matrices stack on the *same* shared model as hi/ne/ps, exactly like every other language. One `SeamlessASR` instance now serves all 7 languages (peak VRAM 3.32 GB, down from an earlier two-instance implementation's 6.79 GB).
